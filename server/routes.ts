...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/meals/:id", async (req, res) => {
    ...
    const meal = storage.meals.get(id);
    if (!meal) {
      return res.status(404).json({ message: "Meal not found" });
    }

    const updatedMeal = { ...meal, ...updatedData };
    storage.meals.set(id, updatedMeal);
    return res.json(updatedMeal);
  });

  // Edit a specific workout by ID
  app.put("/api/workouts/:id", async (req, res) => {
    const id = parseInt(req.params.id);
    const updatedData = req.body;

    const workout = storage.workouts.get(id);
    if (!workout) {
      return res.status(404).json({ message: "Workout not found" });
    }

    const updatedWorkout = { ...workout, ...updatedData };
    storage.workouts.set(id, updatedWorkout);
